...
\begin{figure}[!h]
    \centering
    \includegraphics[width=10cm,height=7cm]{report/figures/modeling-schema.jpg}
...
La classification basées sur les shapelets ont trois avantages \citep{ye2009time}:
\begin{itemize}
    ...
    \item la rapidité de classification: les shapelets étant des sous-séquences de taille généralement beaucoup plus petite que la taille des séries à classer, il est beaucoup plus rapide de comparer les séries aux shapelets que des comparer les séries entre elles.
\end{itemize}

Pour pourvoir appliquer ce modèle il faut pourvoir extraire les shapelets définir la notion de similarité entre un shapelet et une série temporelle.
